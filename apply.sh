...
original_author="cdat13"
original_name="ticketselling"
original_urlname="TicketSelling"
original_description="Awesome ticketselling created by cdat13" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/ticketselling "${name}"
...
then
    ...
    echo "then run 'ticketselling' or 'python -m ticketselling'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'ticketselling' or 'python -m ticketselling' or 'poetry run ticketselling'"
fi
